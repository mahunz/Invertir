...
def getfinancialreportingdf(ticker):
    ...
    titlesyear = text_soup_financials.findAll('th', {'class': 'rowTitle'})
    ...
    for title in titlesfinancials:
        ...
        if 'Interest Expense' in title.text:
            interestexpenselist.append ([td.text for td in title.findNextSiblings(attrs={'class': 'valueCell'}) if td.text])  # No todos los balances figura igual
#        if 'Interest Expense, Net of Interest Capitalized' in title.text:
#            interestexpenselist.append([td.text for td in title.findNextSiblings(attrs={'class': 'valueCell'}) if td.text])
    ...
    for title in titlesyear:
        if 'year' in title.text:
            yearlist.append([th.text for th in title.findNextSiblings(attrs={'scope': 'col'}) if
                             th.text and th.text != '5-year trend'])
    ...
    df= pd.DataFrame({'eps': eps,'epsgrowth': epsgrowth,'netincome': netincome,'shareholderequity': shareholderequity,'roa': 
                  roa,'longtermdebt': longtermdebt,'interestexpense': interestexpense,'ebitda': ebitda},index=yearlist[0])

#    print('interes expense')
#    print(df['interestexpense'])
    dfformatted = df.apply(format)

    # Adding roe, interest coverage ratio
    df['roe'] = (dfformatted.netincome / dfformatted.shareholderequity)*100

    df['interestcoverageratio'] = (dfformatted.ebitda / dfformatted.interestexpense)

#    print (dfformatted.interestexpense )
#    print(dfformatted.ebitda)
#    print (df['interestcoverageratio'] )

    df['roe']=df['roe'].round(2).apply(str) + '%'
    df['interestcoverageratio'] = df['interestcoverageratio'].round(2)
...
def save_russell_info():
    ...
    dfrussel=pd.read_csv('D:/Martin PC/Invertir/RussellandData.csv',index_col='Symbol')
...
def save_self_stocks_info():
    ...
    dictlist.append({'value': 'ypf', 'label': 'YPF S.A. ADR'})
